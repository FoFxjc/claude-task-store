/**
 * Compact resume-context renderer.
 *
 * Extracted from `core.ts` as part of #25 (structural decomposition).
 * `buildResumeContext` projects the active topic of a {@link TaskState}
 * into a bounded, token-budgeted text blob suitable for injecting at
 * session start. `fitResumeToBudget` enforces the hard character budget
 * on any pre-rendered text.
 *
 * Both functions are pure. The module owns:
 *
 *   - the public budget constants {@link RESUME_BUDGET_CHARS} and
 *     {@link RESUME_TRUNCATION_SUFFIX};
 *   - the field-bounded truncation helper `bound`;
 *   - the eight section renderers (header, current, blocked, next
 *     action, done, remaining, decisions, footer);
 *   - the two-pass assembly that keeps priority sections (header,
 *     current, blocked, next action, footer) at full size while
 *     softly capping or compressing lower-priority ones;
 *   - the global omission marker that surfaces when any section was
 *     collapsed, soft-capped, or field-bounded.
 *
 * Token counts are approximate (no model-specific tokenizer). The
 * postcondition `result.length <= RESUME_BUDGET_CHARS` always holds for
 * the complete returned string. No module-level mutable state — safe
 * for concurrent / re-entrant calls.
 */
import { getActiveTopic } from './codec.js';
import type { Decision, Task, TaskState, TopicState } from './types.js';

/**
 * Hard character budget for the resume projection. The COMPLETE
 * returned string — including any global omission marker or truncation
 * marker — never exceeds this value.
 */
 ...
 * it is returned unchanged. Otherwise, the suffix is appended at a
 * complete-line boundary: find the last newline in the budget-allowable
 * prefix, truncate there, and append the suffix. If even the suffix
 * alone exceeds the budget (e.g. the budget was tightened below the
 * suffix length), the suffix is clamped to the budget and returned
 * as-is.
 *
 * Postcondition: `result.length <= budget` always holds. No partial
 ...
 *
 * Every section that is not rendered in full is replaced with a
 * one-line marker naming the section, identifying the omitted content
 * (count, and task IDs for CURRENT/BLOCKED), and pointing at
 * `task-store status`. No omission is ever silent.
 *
